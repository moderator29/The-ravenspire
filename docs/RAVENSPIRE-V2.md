...
## 50. Gasless and forgiving, and the wire that turned out not to exist
...
### 50.1 What account abstraction on Privy actually is, which is not what the brief assumed
...
### 50.2 What it costs, and whether a free tier carries us
...
Two honest caveats. The provider numbers above are from public pricing pages and
the founder should confirm them on the CDP dashboard before flipping anything,
because a 7% surcharge on an unbounded overflow is precisely the surprise invoice
rule 19 exists to prevent. And the gas estimate is a live number: if Base gets
...
### 50.3 The division that makes the degradation honest

This is the design, and it is forced by 50.1 rather than chosen.
...
### 50.4 The wallet cap is not the commerce cap, and the answer is no
...
### 50.5 Recovery: what Privy offers versus what the brief assumed
...
### 50.6 The budget is counted in acts, and why
...
dollars to acts is a sum the founder does once, and it is written out in 50.2 so
...
### 50.7 Exactly what is dark, and what each piece waits on
...
**The migration is written and not applied**, per the standing rule. Two tables,
...
which is the point of 50.4.

### 50.8 What was found on the way
