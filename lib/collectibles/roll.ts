...
import { sha256Hex, hmacSha256Hex } from "@/lib/collectibles/sha256";

/* What comes out of a chest, and why anyone should believe it.
 *
 * THIS FILE USED TO BE `pulls.ts` AND USED TO BE SERVER ONLY. It is neither now,
 * and the move is the whole of mission 6. The chest has always been provably
 * fair: the seed is committed before the member acts, revealed when the chest
 * opens, and the roll is a pure function of three inputs. What it did not have
 * was anywhere a sceptic could actually run the rerun. A verifier that asks the
 * realm's own server to recompute the draw and then reports "yes, it matches"
 * is the realm marking its own homework, which proves nothing about a realm
 * that has decided to lie.
 *
 * So the algorithm ships to the browser. `lib/collectibles/verify.ts` and the
 * page at `/proof` execute exactly this module on the member's own machine, and
 * `pulls.ts` is now a server only re-export of it, so there is one
 * implementation and not two. Two would be the trap: the day they disagreed,
 * the verifier would call an honest opening a forgery, and nothing could tell
 * that from the other direction.
 *
 * The digests come from `lib/collectibles/sha256.ts` rather than `node:crypto`
 * for the same reason. See that file for why a native digest was the wrong
 * trade.
 *
 ...
 *                 chest opens. Stops the member predicting the roll.
...
function draw(
  ...
  const digest = hmacSha256Hex(serverSeed, `${clientSeed}:${nonce}:${label}`);
