...
/* The guardrails were applied in four parts, so they live in four files, and
   this reads whichever ones are there rather than one name. Naming a single
   file would make this test a hostage to how the migration happened to be
   applied: the day it is split again, the test stops finding the functions and
   fails for a reason that has nothing to do with the joint it is guarding.
   A glob over the directory cannot go stale that way. */
const HERE = fileURLToPath(new URL(".", import.meta.url));
const MIGRATIONS_DIR = `${HERE}../../supabase/migrations`;
const MIGRATION = readdirSync(MIGRATIONS_DIR)
  .filter((f) => f.includes("compliance_guardrails") && f.endsWith(".sql"))
  .sort()
