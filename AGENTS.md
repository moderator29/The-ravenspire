...
7. Ticker is `$RSP`, total supply 1,000,000,000, matching the standard
   ...
   describe it as open or running in present tense anywhere while the flag
   is off. When live, it offers 7 percent of supply at a fixed rate,
   non-custodial and wallet to wallet. `lib/season-zero.ts` is the single
   source of truth for its numbers. Any later sale phases will be announced
   before they run. Show POINTS for earned balances, never $RSP amounts; the
   Season Zero allocation is purchased, not earned, so showing its $RSP
   amount is consistent with that rule.
