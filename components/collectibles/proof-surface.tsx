...
/* THE VERIFIER, at /proof and at /proof/<reference>. Deliberately OUTSIDE the
 * shell, and that is a decision rather than a routing accident.
 *
 * It became a component in mission 10 so that one opening could have its own
 * address, and therefore its own Open Graph card, without the explainer above
 * the verifier being written twice. A stranger who is handed a link to somebody
 * else's chest and a member who came here to check their own are reading the
 * same argument, and the argument only works if it is the same one.
 *
...
export function ProofSurface({ reference }: { reference?: string }) {
  return (
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-5 px-3 py-4 sm:px-4 sm:py-6">
        <div className="flex items-center justify-between gap-3">
          {/* A visitor who arrived cold has no realm behind them, so the
              fallback is the gate rather than a page they cannot enter. From a
              single opening the way back is the verifier itself, which is a
              real parent rather than the landing page. */}
          <BackButton href={reference ? "/proof" : "/"} />
          {reference ? (
            <ShareButton
              target={{ kind: "proof", reference }}
              label="Share this proof"
            />
          ) : null}
        </div>
        ...
        <Suspense fallback={null}>
          <DrawVerifier reference={reference} />
        </Suspense>
